  ...
  group("constraints", () {
    ...
    group("apply", () {
      ...
      test("collapses nodes with prefRank=min", () {
        ...
        expect(g.edge(eMin1), propertyVal("minLen", 2));
        expect(g.edge(eMin1), propertyVal("reversed", true));

        expect(g.outEdges(min, 4).length, equals(1));
        var eMin4 = g.outEdges(min, 4);
        expect(g.edge(eMin4), propertyVal("minLen", 4));
        expect(g.edge(eMin4), notProperty("reversed"));
      });
      ...
      test("collapses nodes with prefRank=max", () {
        ...
        expect(g.edge(eMax1), propertyVal("minLen", 2));
        expect(g.edge(eMax1), notProperty("reversed"));

        expect(g.outEdges(4, max).length, equals(1));
        var eMax4 = g.outEdges(4, max);
        expect(g.edge(eMax4), propertyVal("minLen", 4));
        expect(g.edge(eMax4), propertyVal("reversed", true));
      });
      ...
      test("collapses nodes with prefRank=same_x", () {
        ...
        expect(g.edge(eSame1), propertyVal("minLen", 2));

        expect(g.outEdges(x, 4).length, equals(1));
        var eSame4 = g.outEdges(x, 4);
        expect(g.edge(eSame4), propertyVal("minLen", 4));
      });
    ...
    group("relax", () {
      test("restores expands collapsed nodes and sets the rank on expanded nodes", () {
        ...
        expect(g.nodes(), same([1, 2, 3, 4]));
        expect(g.node(1), propertyVal("rank", 0));
        expect(g.node(2), propertyVal("rank", 2));
        expect(g.node(3), propertyVal("rank", 2));
        expect(g.node(4), propertyVal("rank", 6));
        expect(g.edges(), same(["A", "B"]));
      ...
      test("restores expanded nodes to their original subgraph", () {
        ...
        g.node(1).rank = 0;
        g.node(g.children("sg1").filter((u) { return u != 1; })[0]).rank = 2;
        g.node(4).rank = 0;
        g.node(g.children("sg2").filter((u) { return u != 4; })[0]).rank = 2;

        constraints.relax(g);

        expect(g.children("sg1"), same([1, 2, 3]));
        expect(g.children("sg2"), same([4, 5, 6]));
      });
    });
  });
}